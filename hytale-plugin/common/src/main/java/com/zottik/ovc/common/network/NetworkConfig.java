package com.zottik.ovc.common.network;
...
public class NetworkConfig {
    ...
    public static final boolean DEFAULT_USE_PROXIMITY_RADAR_SPEAKING_ONLY = false;
    ...
    private static boolean useProximityRadarSpeakingOnly = DEFAULT_USE_PROXIMITY_RADAR_SPEAKING_ONLY;
    ...
    static {
        try {
            ...
            signalingPort = com.zottik.ovc.common.config.VoiceConfig.getInt("SignalingPort", signalingPort);
            enableSSL = com.zottik.ovc.common.config.VoiceConfig.getBoolean("EnableSSL", enableSSL);
            ...
            maxVoiceDistance = com.zottik.ovc.common.config.VoiceConfig.getDouble("MaxVoiceDistance", maxVoiceDistance);
            positionSampleIntervalMs = com.zottik.ovc.common.config.VoiceConfig.getInt("PositionSampleIntervalMs", positionSampleIntervalMs);
            positionBroadcastIntervalMs = com.zottik.ovc.common.config.VoiceConfig.getInt("PositionBroadcastIntervalMs", positionBroadcastIntervalMs);
            positionMinDistanceDelta = com.zottik.ovc.common.config.VoiceConfig.getDouble("PositionMinDistanceDelta", positionMinDistanceDelta);
            positionRotationThresholdDeg = com.zottik.ovc.common.config.VoiceConfig.getDouble("PositionRotationThresholdDeg", positionRotationThresholdDeg);
            volumeProcessingMode = com.zottik.ovc.common.config.VoiceConfig.getString("VolumeProcessingMode", volumeProcessingMode);
            groupGlobalVoice = com.zottik.ovc.common.config.VoiceConfig.getBoolean("GroupGlobalVoice", groupGlobalVoice);
            groupSpatialAudio = com.zottik.ovc.common.config.VoiceConfig.getBoolean("GroupSpatialAudio", groupSpatialAudio);
            groupMinVolume = com.zottik.ovc.common.config.VoiceConfig.getDouble("GroupMinVolume", groupMinVolume);
            useProximityRadar = com.zottik.ovc.common.config.VoiceConfig.getBoolean("USE_PROXIMITY_RADAR", useProximityRadar);
            useProximityRadarSpeakingOnly = com.zottik.ovc.common.config.VoiceConfig.getBoolean("USE_PROXIMITY_RADAR_SPEAKING_ONLY", useProximityRadarSpeakingOnly);
            gameQuitGraceSeconds = com.zottik.ovc.common.config.VoiceConfig.getInt("GameQuitGraceSeconds", gameQuitGraceSeconds);
            pendingGameJoinTimeoutSeconds = com.zottik.ovc.common.config.VoiceConfig.getInt("PendingGameJoinTimeoutSeconds", pendingGameJoinTimeoutSeconds);
            stunServers = com.zottik.ovc.common.config.VoiceConfig.getString("StunServers", stunServers);
            turnServers = com.zottik.ovc.common.config.VoiceConfig.getString("TurnServers", turnServers);
            icePortMin = com.zottik.ovc.common.config.VoiceConfig.getInt("IcePortMin", icePortMin);
            icePortMax = com.zottik.ovc.common.config.VoiceConfig.getInt("IcePortMax", icePortMax);
            enableOpusDataChannel = com.zottik.ovc.common.config.VoiceConfig.getBoolean("EnableOpusDataChannel", enableOpusDataChannel);
            opusFrameDurationMs = com.zottik.ovc.common.config.VoiceConfig.getInt("OpusFrameDurationMs", opusFrameDurationMs);
            opusSampleRate = com.zottik.ovc.common.config.VoiceConfig.getInt("OpusSampleRate", opusSampleRate);
            opusChannels = com.zottik.ovc.common.config.VoiceConfig.getInt("OpusChannels", opusChannels);
            opusTargetBitrate = com.zottik.ovc.common.config.VoiceConfig.getInt("OpusTargetBitrate", opusTargetBitrate);
            
            // Fallback to old property names for backward compatibility
            signalingPort = com.zottik.ovc.common.config.VoiceConfig.getInt("voice.signaling.port", signalingPort);
            enableSSL = com.zottik.ovc.common.config.VoiceConfig.getBoolean("voice.ssl.enabled", enableSSL);
            ...
            maxVoiceDistance = com.zottik.ovc.common.config.VoiceConfig.getDouble("voice.proximity.max", maxVoiceDistance);
            positionSampleIntervalMs = com.zottik.ovc.common.config.VoiceConfig.getInt("voice.position.sample.interval.ms", positionSampleIntervalMs);
            positionBroadcastIntervalMs = com.zottik.ovc.common.config.VoiceConfig.getInt("voice.position.broadcast.interval.ms", positionBroadcastIntervalMs);
            positionMinDistanceDelta = com.zottik.ovc.common.config.VoiceConfig.getDouble("voice.position.min.distance.delta", positionMinDistanceDelta);
            positionRotationThresholdDeg = com.zottik.ovc.common.config.VoiceConfig.getDouble("voice.position.rotation.threshold.deg", positionRotationThresholdDeg);
            volumeProcessingMode = com.zottik.ovc.common.config.VoiceConfig.getString("voice.volume.processing", volumeProcessingMode);
            groupGlobalVoice = com.zottik.ovc.common.config.VoiceConfig.getBoolean("voice.group.global", groupGlobalVoice);
            groupSpatialAudio = com.zottik.ovc.common.config.VoiceConfig.getBoolean("voice.group.spatial", groupSpatialAudio);
            groupMinVolume = com.zottik.ovc.common.config.VoiceConfig.getDouble("voice.group.minvolume", groupMinVolume);
            useProximityRadar = com.zottik.ovc.common.config.VoiceConfig.getBoolean("voice.ui.proximity.radar.enabled", useProximityRadar);
            useProximityRadarSpeakingOnly = com.zottik.ovc.common.config.VoiceConfig.getBoolean("voice.ui.proximity.radar.speaking.only", useProximityRadarSpeakingOnly);
            gameQuitGraceSeconds = com.zottik.ovc.common.config.VoiceConfig.getInt("voice.game.quit.grace.seconds", gameQuitGraceSeconds);
            pendingGameJoinTimeoutSeconds = com.zottik.ovc.common.config.VoiceConfig.getInt("voice.game.join.pending.timeout.seconds", pendingGameJoinTimeoutSeconds);
            stunServers = com.zottik.ovc.common.config.VoiceConfig.getString("voice.webrtc.stun.servers", stunServers);
            turnServers = com.zottik.ovc.common.config.VoiceConfig.getString("voice.webrtc.turn.servers", turnServers);
            icePortMin = com.zottik.ovc.common.config.VoiceConfig.getInt("voice.webrtc.ice.port.min", icePortMin);
            icePortMax = com.zottik.ovc.common.config.VoiceConfig.getInt("voice.webrtc.ice.port.max", icePortMax);
            enableOpusDataChannel = com.zottik.ovc.common.config.VoiceConfig.getBoolean("voice.webrtc.opus.enabled", enableOpusDataChannel);
            opusFrameDurationMs = com.zottik.ovc.common.config.VoiceConfig.getInt("voice.webrtc.opus.frame.duration.ms", opusFrameDurationMs);
            opusSampleRate = com.zottik.ovc.common.config.VoiceConfig.getInt("voice.webrtc.opus.sample-rate", opusSampleRate);
            opusChannels = com.zottik.ovc.common.config.VoiceConfig.getInt("voice.webrtc.opus.channels", opusChannels);
            opusTargetBitrate = com.zottik.ovc.common.config.VoiceConfig.getInt("voice.webrtc.opus.target-bitrate", opusTargetBitrate);
        } catch (Exception e) {
    ...
    /**
     * Check if proximity radar should display only speaking players.
     */
    public static boolean isProximityRadarSpeakingOnlyEnabled() {
        return useProximityRadarSpeakingOnly;
    }
