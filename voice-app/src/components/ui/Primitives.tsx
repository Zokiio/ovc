...
export const Modal = ({ isOpen, onClose, title, children }: { isOpen: boolean, onClose: () => void, title: string, children: React.ReactNode }) => {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
      ...
      <Panel 
        className="w-full max-w-md relative animate-in zoom-in-95 duration-300 z-10 border-[var(--border-active)]" 
        title={title}
        rightElement={
          <button 
            onClick={onClose} 
            className="p-1 text-[var(--text-secondary)] hover:text-[var(--accent-danger)] transition-colors"
